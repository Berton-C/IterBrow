...
def _load():
    # Resilience fix (E33, 2026-09-13): a valid-but-empty dict (e.g. {})
    # must also fall back to seeds, or the registry silently degrades.
    try:
        with open(SKILLS_PATH) as f:
            data = json.load(f)
        if not isinstance(data, dict) or (not data.get("skills") and not data.get("self_authored")):
